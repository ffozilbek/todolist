...
window.addEventListener("DOMContentLoaded", ()=> {
    ...
    elForm.addEventListener("submit", (e)=> {
        ...
        if(elInput.value) {
            ...
            createElItem.classList.add("todo__item");

            // ADD LOCATSTORAGE DATA
            const formData = new FormData(elForm);
            const objTodo = {};
            formData.forEach((item,key) => objTodo[key]=item)

            const db = JSON.parse(localStorage.getItem("posts"));
            if(db) localStorage.setItem("posts", JSON.stringify([...db,objTodo]))
            else localStorage.setItem("posts", JSON.stringify([objTodo]))
        ...
        elTodoList.append(createElItem);
